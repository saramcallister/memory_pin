...
std::unordered_map<uint64_t, uint64_t> traceToMap(TraceFile &trace, int64_t instr_limit = std::numeric_limits<int64_t>::max()) {
...
std::vector<uint64_t> getStartingLFU(std::unordered_map<uint64_t, uint64_t> map, uint64_t total_pages) {

    std::priority_queue <PageEntry, std::vector<PageEntry>, PageEntryComparator> pq;
    for (auto & [ page_num, accesses ] : map) {
...
int main(int argc, char* argv[]) {
    ...
    double scale = std::stof(argv[5]);   // in seconds


    int arg_num = 6;
    std::vector<int> page_mvmt_speeds{1000, 5000, 10000, 50000, 100000}; // 1 page mvmt per x ms
//     std::vector<int> page_mvmt_speeds{1000}; // 1 page mvmt per x ms
    while (arg_num < argc) {
...
//             fprintf(stderr, "page_limit: %lu, mvmt speed: %lu\n", page_limit,
//             timing); 
            pinTrace = TraceFile();
            pinTrace.setTraceFile(argv[1]);
