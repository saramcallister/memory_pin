...
std::vector<uint64_t> getStartingRandom(TraceFile &trace, uint64_t total_pages, 
...
timing, int num_bits, uint64_t sample_time, int scale) {
...
//     std::vector<int> page_mvmt_speeds{10, 100, 1000, 5000, 10000, 100000}; // 1 page mvmt per x ms
    ...
    while (arg_num < argc) {
        ...
        getStartingRandom(pinTrace, page_limit);
